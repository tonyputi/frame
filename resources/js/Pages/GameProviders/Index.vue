            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div v-if="gameProviders.meta.total > 0" class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <jet-table class="text-sm">
                        ...
                        <template #body>
                            <tr v-for="resource in gameProviders.data" :key="resource.id" class="border border-black-600">
                                <td class="px-2 py-4 text-center">
                                    <jet-checkbox :value="resource" v-model:checked="gameProvidersSelected" />
                                </td>
                                <td class="px-2 py-4 text-left">{{ resource.id }}</td>
                                <td class="px-2 py-4 text-left">
                                    <img class="h-8 w-8 rounded-full object-cover" :src="resource.logo_url" :alt="resource.name" />
                                </td>
                                <td class="px-2 py-4 text-left">
                                    <svg v-if="resource.candidate_game_provider_on_queue.is_active" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-5 w-5">
                                ...
                                <td class="px-2 py-4 text-left">
                                    <inertia-link :href="route('game-provider-queues.index')">
                                        {{ resource.game_provider_queues_count }}
                                    </inertia-link>
                                </td>
                                <td class="px-2 py-4 text-left">{{ resource.candidate_game_provider_on_queue.user?.name }}</td>
                                <td class="px-2 py-4 text-left">{{ formatDate(resource.candidate_game_provider_on_queue.started_at) }}</td>
                                <td class="px-2 py-4 text-left">{{ formatDate(resource.candidate_game_provider_on_queue.ended_at) }}</td>
                                <td class="px-2 py-4 text-center">
                                    <div class="flex">
                                        ...
                                        <inertia-link class="text-black-500 ml-4" :href="route('game-providers.show', resource.id)">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-5 w-5">
                    ...
                    <pagination v-bind="gameProviders.meta" />
                </div>
